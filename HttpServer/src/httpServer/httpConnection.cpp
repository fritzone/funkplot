...
void HttpConnection::createSocket(qintptr socketDescriptor)
{
    ...
    if (config->verbosity >= HttpServerConfig::Verbose::Debug)
        qDebug() << QString("New incoming connection from %1").arg(socket->peerAddress().toString());
...
void HttpConnection::read()
{
	...
	while (socket->bytesAvailable())
    {
        ...
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
            qInfo() << QString("Received %1 request to %2 from %3").arg(currentRequest->method())
        ...
        promise
            ...
            .finally([=]() {
                ...
                if (!response->isValid())
                {
                    ...
                    {
                        qWarning() << QString("No valid response set, defaulting to 500: %1 %2 %3")
...
{
    if (config->verbosity >= HttpServerConfig::Verbose::Debug)
        qDebug() << QString("Client %1 disconnected").arg(address.toString());
...
void HttpConnection::sslErrors(const QList<QSslError> &errors)
{
    if (config->verbosity >= HttpServerConfig::Verbose::Warning)
    {
        ...
        qWarning() << QString("TLS handshake failed for client %1: %2").arg(address.toString()).arg(errorMessages);
    }
