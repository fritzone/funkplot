...
bool HttpRequest::parseRequestLine(QTcpSocket *socket, HttpResponse *response)
{
    ...
    if (requestBytesSize > config->maxRequestSize)
    {
        ...
        {
            qInfo() << QString("Maximum size for request was reached for %1 (%2)")
    ...
    if (parts.length() != 3 || !parts[2].startsWith("HTTP"))
    {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
        {
            qInfo() << QString("Invalid HTTP request line received from %1: %2").arg(socket->peerAddress().toString())
    ...
    {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
            qInfo() << QString("Invalid method received from %1: %2").arg(address_.toString()).arg(method_);
    ...
    {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
            qInfo() << QString("Invalid URI received from %1: %2").arg(address_.toString()).arg(QString(parts[1]));
...
bool HttpRequest::parseHeader(QTcpSocket *socket, HttpResponse *response)
{
    ...
    if (requestBytesSize > config->maxRequestSize)
    {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
        {
            qInfo() << QString("Maximum size for request was reached for %1 (%2)").arg(address_.toString())
    ...
    if (index == -1)
    {
        if (config->verbosity >= HttpServerConfig::Verbose::Info)
        {
            qInfo() << QString("Invalid headers in request for %1 (%2)").arg(address_.toString())
                .arg(QString(buffer));
        }
    ...
    if (field.compare("Cookie", Qt::CaseInsensitive) == 0)
    {
        ...
        for (QString part : parts)
        {
            ...
            {
                if (config->verbosity >= HttpServerConfig::Verbose::Info)
                    qInfo() << QString("Invalid cookie header for %1: %2").arg(address_.toString()).arg(value);
...
bool HttpRequest::parseMultiFormBody(QTcpSocket *socket, HttpResponse *response)
{
    ...
    if (state_ == State::ReadMultiFormBodyData)
    {
        ...
        const int index = QString(buffer).indexOf(delimiter);
...
void HttpRequest::parseContentType()
{
    ...
    if (match.hasMatch())
    {
        ...
        {
            boundary = boundary.mid(0, boundary.size() - 1);
        }
        return;
    }
    ...
    else
    {
        ...
        if (!codec)
        {
            ...
            if (config->verbosity >= HttpServerConfig::Verbose::Warning)
                qWarning() << QString("Unknown charset when parsing body: %1. Falling back to UTF-8").arg(charset_);
...
QJsonDocument HttpRequest::parseJsonBody() const
{
    ...
    if (config->verbosity >= HttpServerConfig::Verbose::Warning && error.error != QJsonParseError::NoError)
        qWarning() << QString("Unable to parse JSON document: %1").arg(error.errorString());
