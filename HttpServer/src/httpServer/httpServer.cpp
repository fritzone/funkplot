...
bool HttpServer::listen()
{
    if (!QTcpServer::listen(config.host, config.port))
    {
        ...
        {
            qWarning() << QString("Unable to listen on %1:%2: %3").arg(config.host.toString())
...
void HttpServer::loadSslConfig()
{
    ...
    if (!config.sslKeyPath.isEmpty() && !config.sslCertPath.isEmpty())
    {
        if (!QSslSocket::supportsSsl())
        {
            if (config.verbosity >= HttpServerConfig::Verbose::Warning)
            {
                qWarning() << QString("OpenSSL is not supported for HTTP server (OpenSSL Qt build "
        ...
        if (!certFile.open(QIODevice::ReadOnly))
        {
            if (config.verbosity >= HttpServerConfig::Verbose::Warning)
            {
                qWarning() << QString("Failed to open SSL certificate file for HTTP server: %1 (%2). "
        ...
        if (certificate.isNull())
        {
            if (config.verbosity >= HttpServerConfig::Verbose::Warning)
            {
                qWarning() << QString("Invalid SSL certificate file for HTTP server: %1. Disabling TLS")
        ...
        if (!keyFile.open(QIODevice::ReadOnly))
        {
            if (config.verbosity >= HttpServerConfig::Verbose::Warning)
            {
                qWarning() << QString("Failed to open private SSL key file for HTTP server: %1 (%2). "
        ...
        if (sslKey.isNull())
        {
            if (config.verbosity >= HttpServerConfig::Verbose::Warning)
            {
                qWarning() << QString("Invalid private SSL key for HTTP server: %1. Disabling TLS")
        ...
        if (config.verbosity >= HttpServerConfig::Verbose::Debug)
            qDebug() << "Successfully setup SSL configuration, HTTPS enabled";
    }
    else if (config.verbosity >= HttpServerConfig::Verbose::Debug)
        qDebug() << "No private key or certificate file path given. Disabling TLS";
}
